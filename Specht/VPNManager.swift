...
class VPNManager {
    ...
    static func removeAllManagers(completionHandler: @escaping () -> ()) {
        NETunnelProviderManager.loadAllFromPreferences { managers, error in
            guard let managers = managers else {
                appDelegate.alertError(errorDescription: "Failed to load VPN settings from preferences due to \(String(describing: error))")
                return
            }
            ...
            for manager in managers {
                manager.removeFromPreferences { error in
                    if error != nil {
                        appDelegate.alertError(errorDescription: "Failed to remove VPN settings from preferences due to \(String(describing: error))")
                    }
    ...
    static func loadConfigFile(path: String, completionHandler: @escaping () -> ()) {
        ...
        do {
            ...
            configuration.providerConfiguration!["hash"] = MD5.string(s: content)
        } catch let error {
            appDelegate.alertError(errorDescription: "Error when loading config file: \(path). \(error)")
            completionHandler()
            return
        }
    ...
    static func loadAllConfigFiles(configFolder: String, completionHandler: @escaping () -> ()) {
        ...
        for path in paths {
            loadConfigFile(path: (configFolder as NSString).appendingPathComponent(path)) {
                pendingAction -= 1
                if pendingAction == 0 {
                    completionHandler()
                }
